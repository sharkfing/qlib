...
After running the scripts, you can find local training files under the project-level cache directory `datacache/TRA/`. Historical relative `logdir` settings such as `output/Alpha158` are mapped to `datacache/TRA/Alpha158`:
...
MLflow experiment metadata and artifacts remain under `mlruns/`.
